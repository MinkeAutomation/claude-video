...
### Hinzugefügt
- **Kein erzwungener Player-Client mehr (Standard), die alte Absicherung ist ueber `WATCH_PLAYER_CLIENTS` zurueckholbar.** `download.py` rief yt-dlp bisher fest mit `player_client=tv,web_safari,mweb` auf. Gemessen am 19.08.2026 an `u2v17HBnhh8`: mit dieser Liste bleibt **ein einziges Format 640x360** uebrig und die Untertitel melden `none available`. Ohne die Liste waehlt yt-dlp selbst `android vr` und bietet **bis 3840x2160** plus die deutschen Auto-Captions, im ersten Versuch. Ursache ist, dass die erzwungenen Clients halb gelingen und yt-dlp deshalb gar nicht auf einen besseren durchfaellt. ⚠️ **`android` einfach anzuhaengen half nicht**, das war der erste Versuch am selben Abend und aenderte nichts. Wer die Absicherung braucht (HTTP 403, "fragment not found"), setzt `WATCH_PLAYER_CLIENTS=tv,web_safari,mweb`.
